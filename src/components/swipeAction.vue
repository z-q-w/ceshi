<template>
	<div class="tui-swipeout-wrap" :style="{ backgroundColor: backgroundColor }">
		<div class="tui-swipeout-item" :class="[isShowBtn ? 'swipe-action-show' : '']" @touchstart="handlerTouchstart"
		 ...
		 @mouseup="handlerTouchend" :style="{ transform: 'translate(' + position.pageX + 'px,0)' }">
			<div class="tui-swipeout-content">
				{{listitem}} {{position.pageX}}
			</div>
            
			<div class="tui-swipeout-button-right-group" v-if="actions.length > 0" @touchend.stop="loop">
				<div class="tui-swipeout-button-right-item" v-for="(item, index) in actions" :key="index" :style="{ backgroundColor: item.background || '#f7f7f7', color: item.color, width: item.width + 'px' }"
				 :data-index="index" @click="handlerButton">
                    <img :src="item.icon" v-if="item.icon" :style="{ width: item.imgWidth + 'px', height: item.imgHeight + 'px' }">
	...
	export default {
		...
		methods: {
			...
			handlerTouchstart(event) {
				...
				if (this.forbid) return;                     //禁止滑动
				let touches = event.touches
				if (touches && touches.length > 1) return;   //这个不知道是干嘛用的  event.touches== undefined 
				this.move = true;                            //这个作用是 安住鼠标向移动  鼠标松开会变为 false
