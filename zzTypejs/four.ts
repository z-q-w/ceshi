...
//平常ts写法
    function dunp(arg:string):string{
        return 'dds'
    }
    let hdf=dunp('houdunren.com')

//动态设置类型
    function hdsfg<t>(arr:t):t{
        return arr
    }
    let gt = hdsfg<string>('ppp')
    let fg = hdsfg<boolean>(true)
...
// 断言 ，可以用来告诉解析器变量的实际类型
    ...
    let sh:string
    fddf='ppp'
    sh=fddf as string //也可以这么写 sh=<string>fddf
...
// object 对象  属性后面加上?表示可选熟悉写不写都可以
    let bo:{name:string,age?:number}
    bo={
        name:'张三',
        age:13
    }
//[prooname:string]:any 表示任意类型的属性  可以随意添加属性
    let cof:{name:string,[prooname:string]:any}
    cof={
        name:'李四',
        age:13,
        sex:'难'
